...
class InstallReahlServer:
    ...
    def __init__(self, p_ini_pth, p_logger=None):
        ...
        self.reahl_distribution_dir = Path(self.ini.get('DEFAULT', 'ReahlDistributionFolder'))
    ...
    def create_db(self, p_reahl_app_name):
        ...
        if self.ini.getboolean(p_reahl_app_name, 'CreateDb'):
            ...
            if self.ini.get(p_reahl_app_name, 'CreateDbUser'):
                script_cmds.append(f'reahl createdbuser {self.reahl_config_dir / p_reahl_app_name}')
            if self.ini.get(p_reahl_app_name, 'CreateDb'):
                script_cmds.append(f'reahl createdb {self.reahl_config_dir / p_reahl_app_name}')
            if self.ini.get(p_reahl_app_name, 'CreateDbTables'):
                script_cmds.append(f'reahl createdbtables {self.reahl_config_dir / p_reahl_app_name}')
    ...
    def create_linux_users(self):
        ...
        success = True
        if self.curr_os == beeutils.LINUX:
            users = [x[1] for x in self.ini.get('LinuxUsers', self.user_prefix, p_prefix=True, p_split=True)]
    ...
    def create_mysql_users(self):
        ...
        users = [x[1] for x in self.ini.get('MySQLUsers', self.user_prefix, p_prefix=True, p_split=True)]
        success = self.inst_tls.create_mysql_users(admin, users, p_verbose=True) and success
        rights = [x[1] for x in self.ini.get('MySQLUsers', self.mysql_rights_prefix, p_prefix=True, p_split=True)]
        success = self.inst_tls.grant_mysql_user_rights(admin, rights, p_verbose=True) and success
        return success
    ...
    def config_reahl_domain(self, p_reahl_app_name):
        ...
        self.inst_tls.create_reahl_mailutil_config(p_reahl_app_name, self.reahl_config_dir, settings, p_verbose=True)
        pass
    ...
    def _get_create_db_script(self, p_db_system):
        ...
        if p_db_system == 'sqlite':
            ...
            if self.curr_os == beeutils.LINUX:
                ...
                script_cmds.append(f'SET MYSQL_PWD={admin[1]}\n')
            ...
            print(beearchiver.msg_error(f'Unknown database system: {p_db_system}\nSystem terminated...'))
            sys.exit()
    ...
    def install(self):
        ...
        for reahl_wheel in self.ini.get('ReahlWheels', self.package_prefix, p_prefix=True, p_split=False):
            reahl_app_name = self.inst_tls.get_reahl_app_name(reahl_wheel[1])
            beescript.exec_cmd(['python3', '-m', 'venv', self.make_reahl_env_dir(reahl_app_name)])
            ...
            if self.curr_os == beeutils.LINUX:
                ...
                beescript.exec_cmd(['sudo', 'chown', 'www-data.www-data', reahl_app_www_pth])
        ...
        for domain_name in self.ini.get('Domains', self.domain_name_prefix, p_prefix=True, p_split=False):
            self.create_nginx_config(domain_name[1])
        batch = [
            x[1]
            for x in self.ini.get(
                f'{self.batch_name_prefix}03',
                self.command_name_prefix,
                p_prefix=True,
                p_split=True,
            )
    ...
    def install_reahl_prereq_packages(self, p_app_name):
        ...
        if self.curr_os == beeutils.LINUX:
            script_cmds.append('exit')
            script_cmds.append('_EOF_')
        beescript.exec_batch_in_session(script_cmds, p_script_name=script_name, p_verbose=True, p_switches=switches)
        pass
    ...
    def make_reahl_env_dir(self, p_app_name):
        ...
        """
        return self.venv_base_dir / Path(f'{p_app_name}{self.venv_suffix}')
    ...
    def _prepare_test(self):
        ...
        if self.curr_os == beeutils.WINDOWS:
            ...
            for pth in list_to_del:
                ...
                print(beearchiver.msg_info(f'del {pth}'))

        admin = self.ini.get('MySQLUsers', 'Admin', p_split=True)
        users = [[x[1][0], x[1][2]] for x in self.ini.get('MySQLUsers', self.user_prefix, p_prefix=True, p_split=True)]
    ...
    def reahl_activate_env(self, p_app_name):
        ...
        if self.curr_os == beeutils.LINUX:
            script_cmd = 'source {}'.format(self.make_reahl_env_dir(p_app_name) / 'bin' / 'activate')
        else:
            script_cmd = '{}'.format(self.make_reahl_env_dir(p_app_name) / 'Scripts' / 'activate')
        return script_cmd
    ...
    def secure_mysql(self):
        ...
        script_cmds = self.inst_tls.make_sql_base_cmd(admin) + [
            f"ALTER USER 'root'@'localhost' IDENTIFIED WITH mysql_native_password BY '{admin[1]}';"
        ]
        beescript.exec_cmd(script_cmds, p_verbose=True)
        script_cmds = self.inst_tls.make_sql_base_cmd(admin) + ["DELETE FROM mysql.user WHERE User='';"]
        beescript.exec_cmd(script_cmds, p_verbose=True)
        script_cmds = self.inst_tls.make_sql_base_cmd(admin) + [
            "DELETE FROM mysql.user WHERE User='root' AND Host NOT IN ('localhost', '127.0.0.1', '::1');"
        ]
        beescript.exec_cmd(script_cmds, p_verbose=True)
        script_cmds = self.inst_tls.make_sql_base_cmd(admin) + ['DROP DATABASE IF EXISTS test;']
        beescript.exec_cmd(script_cmds, p_verbose=True)
        script_cmds = self.inst_tls.make_sql_base_cmd(admin) + [
...
def read_args():
    ...
    arg_parser.add_argument('--log', required=False, help='Activate logging', default=False)
